...
Использованы NumPy, pandas, scikit-learn, CatBoost, Optuna и matplotlib; официальная метрика и baseline — из задания.
`quickstart.ipynb` сохранён как исходный пример; точка входа для финальной сдачи — `solution.ipynb`.

Подбор гиперпараметров: Optuna/TPE, 20 попыток; раздел 4 → «Подбор гиперпараметров через Optuna».
